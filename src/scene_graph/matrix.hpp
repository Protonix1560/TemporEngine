...
template <typename T>
concept matrix_element = !std::is_volatile_v<T>;


template <size_t N, matrix_element T, typename Alloc>
requires (N > 0)
class matrix;

template <size_t N, matrix_element T>
class matrix_view;
...
class matrix_iterator {
    public:
        ...
        template <matrix_element U>
        requires std::same_as<U, std::remove_const_t<T>>
        matrix_iterator(const matrix_iterator<N, U>& it) : m_ptr(it.m_ptr), m_begin(it.m_begin) {}

        template <matrix_element U>
        requires std::same_as<U, std::remove_const_t<T>>
        matrix_iterator<N, T>& operator=(const matrix_iterator<N, U>& it) { m_ptr = it.m_ptr; m_begin = it.m_begin; return *this; }

        T& operator[](ptrdiff_t diff) requires (N == 1) { return *(*this + diff); }
        T& operator*() requires (N == 1) { return *m_ptr; }
        T* operator->() requires (N == 1) { return m_ptr; }
        matrix_view<N - 1, T> operator[](ptrdiff_t diff) requires (N > 1) { return *(*this + diff); }
        matrix_view<N - 1, T> operator*() requires (N > 1);

        matrix_iterator<N, T>& operator++() { m_ptr += _step_size(m_sizes); return *this; }
        matrix_iterator<N, T>& operator--() { m_ptr -= _step_size(m_sizes); return *this; }

        matrix_iterator<N, T> operator++(int) { matrix_iterator<N, T> old = *this; ++(*this); return old; }
        matrix_iterator<N, T> operator--(int) { matrix_iterator<N, T> old = *this; --(*this); return old; }

        matrix_iterator<N, T> operator+(ptrdiff_t diff) const { return matrix_iterator<N, T>(std::span(m_sizes), m_ptr + diff * _step_size(m_sizes), m_begin); }
        matrix_iterator<N, T> operator-(ptrdiff_t diff) const { return matrix_iterator<N, T>(std::span(m_sizes), m_ptr - diff * _step_size(m_sizes), m_begin); }
        ptrdiff_t operator-(matrix_iterator<N, const T> it) const { return (m_ptr - it.m_ptr) / _step_size(m_sizes); }

        matrix_iterator<N, T>& operator+=(ptrdiff_t diff) { m_ptr += diff * _step_size(m_sizes); return *this; }
        matrix_iterator<N, T>& operator-=(ptrdiff_t diff) { m_ptr -= diff * _step_size(m_sizes); return *this; }

        std::strong_ordering operator<=>(matrix_iterator<N, const T> it) const { return m_ptr <=> it.m_ptr; }
        bool operator==(matrix_iterator<N, const T> it) const { return m_ptr == it.m_ptr; }
        bool operator!=(matrix_iterator<N, const T> it) const { return m_ptr != it.m_ptr; }

    private:
        matrix_iterator(std::span<const size_t, N> sizes, T* ptr, T* begin) : m_ptr(ptr), m_begin(begin) {
        ...
        friend struct matrix_iterator<N, const T>;
        friend struct matrix_iterator<N, std::remove_const_t<T>>;
        template <size_t M, matrix_element, typename> requires (M > 0) friend class matrix;
        friend class matrix_view<N, T>;
};


template <size_t N, matrix_element T>
requires (N > 0)
class matrix_view<N, T> {

    public:
        using iterator = matrix_iterator<N, T>;
        using const_iterator = matrix_iterator<N, const T>;
        ...
        matrix_iterator<M, T> mode_m_begin() {
            return matrix_iterator<M, T>(std::span<const size_t, M>(m_sizes.data(), M), mp_data, mp_data);
        }
        template <size_t M> requires (M <= N && M > 0)
        matrix_iterator<M, const T> mode_m_cbegin() const {
            return matrix_iterator<M, const T>(std::span<const size_t, M>(m_sizes.data(), M), mp_data, mp_data);
        }
        template <size_t M> requires (M <= N && M > 0)
        matrix_iterator<M, const T> mode_m_begin() const { return mode_m_cbegin<M>(); }

        template <size_t M> requires (M <= N && M > 0)
        std::reverse_iterator<matrix_iterator<M, T>> mode_m_rbegin() { return mode_m_end<M>(); }
        template <size_t M> requires (M <= N && M > 0)
        std::reverse_iterator<matrix_iterator<M, const T>> mode_m_crbegin() const { return mode_m_cend<M>(); }
        template <size_t M> requires (M <= N && M > 0)
        std::reverse_iterator<matrix_iterator<M, const T>> mode_m_rbegin() const { return mode_m_crbegin<M>(); }
        ...
        template <size_t M> requires (M <= N && M > 0)
        matrix_iterator<M, T> mode_m_end() {
            return matrix_iterator<M, T>(
        ...
        template <size_t M> requires (M <= N && M > 0)
        matrix_iterator<M, const T> mode_m_cend() const {
            return matrix_iterator<M, const T>(
                std::span<const size_t, M>(m_sizes.data(), M), mp_data + product(m_sizes),
                mp_data + product(m_sizes) - product(std::span<const size_t, M>(m_sizes.data(), M))
            );
        }
        template <size_t M> requires (M <= N && M > 0)
        matrix_iterator<M, const T> mode_m_end() const { return mode_m_cend<M>(); }

        template <size_t M> requires (M <= N && M > 0)
        std::reverse_iterator<matrix_iterator<M, T>> mode_m_rend() { return mode_m_begin<M>(); }
        template <size_t M> requires (M <= N && M > 0)
        std::reverse_iterator<matrix_iterator<M, const T>> mode_m_crend() const { return mode_m_cbegin<M>(); }
        template <size_t M> requires (M <= N && M > 0)
        std::reverse_iterator<matrix_iterator<M, const T>> mode_m_rend() const { return mode_m_crend<M>(); }

        matrix_view<N - 1, T> operator[](size_t index) requires (N > 1) {
            assert(index < m_sizes.back());
            return matrix_view<N - 1, T>(std::span<const size_t, N - 1>(m_sizes.data(), N - 1), mp_data + _step_size(m_sizes) * index);
        }
        ...
        matrix_view<N - 1, T> front() requires (N > 1) { assert(m_sizes.back() > 0); return (*this)[0]; }
        T& front() requires (N == 1) { assert(m_sizes.back() > 0); return (*this)[0]; }

        matrix_view<N - 1, T> back() requires (N > 1) { assert(m_sizes.back() > 0); return (*this)[m_sizes.back() - 1]; }
        T& back() requires (N == 1) { assert(m_sizes.back() > 0); return (*this)[m_sizes.back() - 1]; }

        friend void swap(matrix_view<N, T> a, matrix_view<N, T> b) {
    ...
    private:
        matrix_view(std::span<const size_t, N> sizes, T* p_data) : mp_data(p_data) {
            std::ranges::copy(sizes, m_sizes.begin());
        }
        std::array<size_t, N> m_sizes;
        T* mp_data;
        ...
        template <size_t M, matrix_element, typename> requires (M > 0) friend class matrix;
        friend class matrix_iterator<N + 1, T>;
};


template <size_t N, matrix_element T>
requires (N > 0)
matrix_view<N - 1, T> matrix_iterator<N, T>::operator*() requires (N > 1) {
    return matrix_view<N - 1, T>(std::span<const size_t, N - 1>(m_sizes.data(), N - 1), m_ptr);
}


template <size_t N, matrix_element T, typename Alloc = std::allocator<T>>
requires (N > 0)
class matrix {
    ...
    public:
        ...
        matrix(std::initializer_list<size_t> sizes) {
            assert(sizes.size() == N);
            std::ranges::copy(sizes, m_sizes.begin());
            m_storage.resize(product(m_sizes));
        }
        ...
        matrix_view<N - 1, T> operator[](size_t index) requires (N > 1) {
            assert(index < m_sizes.back());
            return matrix_view<N - 1, T>(std::span<const size_t, N - 1>(m_sizes.data(), N - 1), m_storage.data() + _step_size(m_sizes) * index);
        }
        ...
        template <size_t M> requires (M <= N && M > 0)
        matrix_iterator<M, T> mode_m_begin() {
            return matrix_iterator<M, T>(std::span<const size_t, M>(m_sizes.data(), M), m_storage.data(), m_storage.data());
        }
        template <size_t M> requires (M <= N && M > 0)
        matrix_iterator<M, const T> mode_m_cbegin() const {
            return matrix_iterator<M, const T>(std::span<const size_t, M>(m_sizes.data(), M), m_storage.data(), m_storage.data());
        }
        template <size_t M> requires (M <= N && M > 0)
        matrix_iterator<M, const T> mode_m_begin() const { return mode_m_cbegin<M>(); }
        ...
        template <size_t M>
        requires (M <= N && M > 0)
        inline void erase_hyperplane(matrix_iterator<M, T> pos) {
        ...
        matrix_iterator<M, T> erase_front() { erase_hyperplane(mode_m_begin<M>()); return mode_m_begin<M>(); }

        template <size_t M = N> requires (M <= N && M > 0)
        matrix_iterator<M, T> erase_back() { erase_hyperplane(--mode_m_end<M>()); return mode_m_end<M>(); }
        ...
        template <size_t M>
        inline void insert_hyperplane(matrix_iterator<M, T> pos) {
        ...
        void insert_hyperplane(matrix_iterator<M, T> pos, const U& value) {
        ...
        void emplace_hyperplane(matrix_iterator<M, T> pos, Args&&... args) {
            insert_hyperplane(pos);
            ptrdiff_t step = product(std::span<const size_t, M>(m_sizes.data(), M));
            ptrdiff_t batch_size = product(std::span<const size_t, M - 1>(m_sizes.data() + (N - M), M - 1));
            for (ptrdiff_t i = 0; i < m_storage.size() / step; i++) {
...
template <matrix_element T, typename Alloc = std::allocator<T>, typename... Sizes>
inline matrix<sizeof...(Sizes), T, Alloc> make_matrix(Sizes... sizes) {
    return matrix<sizeof...(Sizes), T, Alloc>(sizes...);
}
